...
\label{sec:background}
	...
	\end{equation}
	where $K := \{k(x_{i}, x_{j})\}_{i = 1, j = 1}^{n, n}$, $\Phi := \begin{bmatrix} \phi(x_{1}) & \dots & \phi(x_{n}) \end{bmatrix}$, $\Psi := \begin{bmatrix} \psi(y_{1}) & \dots & \psi(y_{n}) \end{bmatrix}$, $\phi(x) := k(x, \cdot)$, $\psi(y) := l(y, \cdot)$, and $\lambda$ is the regularisation parameter for the conditional operator \citep{song2013kernel}. The empirical conditional embedding defined by $\hat{\mu}_{Y | X = x} := \hat{\mathcal{U}}_{Y | X} k(x, \cdot)$ then stochastically converges to the conditional embedding $\mu_{Y | X = x}$ in the RKHS norm at a rate of $O_{p}((n \lambda)^{-\frac{1}{2}} + \lambda^{\frac{1}{2}})$, under the assumption that $k(x, \cdot)$ is in the image of $C_{XX}$ \cite[Theorem 6]{song2009hilbert}. This allows us to approximate the conditional expectation \eqref{eq:conditional_expectation} with $\langle \hat{\mu}_{Y | X = x}, g \rangle$ instead, 
	\begin{equation}
...
\label{sec:hyperparameter_learning}
	...
	\end{equation}
	to express classification risk, where we use the notation $[\;\cdot\;]_{\epsilon}^{1} := \min\{\max\{\;\cdot\;, \epsilon\}, 1\}$ for $\epsilon \in (0, 1)$. Under this loss, we prove that the expected risk for the kernel embedding classifier $\bvec{f} = \bvec{f}_{\theta, \lambda}$ \eqref{eq:empirical_decision_probability_vector} is bounded by the empirical risk and a Rademacher complexity bound $r(\theta, \lambda)$ with high probability. %We refer the reader to the supplementary material for detailed proof.
	...
	\paragraph{Toy Example}
		...
		% $ \sigma^{2}_{f} \bvec{Y}^{T} (K_{(\sigma_{f}, \ell)} + n \lambda I)^{-1} K_{(\sigma_{f}, \ell)} (K_{(\sigma_{f}, \ell)} + n \lambda I)^{-1} \bvec{Y} = \bvec{Y}^{T} (K_{(1, \ell)} + n \frac{\lambda}{\sigma^{2}_{f}} I)^{-1} K_{(1, \ell)} (K_{(1, \ell)} + n \frac{\lambda}{\sigma^{2}_{f}} I)^{-1} \bvec{Y}$.
%	RCB from overfitted after 500 iterations: 2.2299
%	RCB from underfitted after 500 iterations: 2.1424
%	
%	HYPERS from overfitted $\sigma_{f} = 0.01$, $\sigma = 0.035$, $\lambda = 10^{-6}$ to $\sigma_{f} = 0.01$, $\sigma = 0.24$, $\lambda = 8.6186 \times 10^{-7}$.
%	
%	$0.195949 / (5.1^2) = 0.007533602460592081 \approx 0.00861677 = 8.61677e-07 / (0.01^2)$
%	
%	HYPERS from underfitted $\sigma_{f} = 1$, $\sigma = 1$, $\lambda = 1$ to $\sigma_{f} = 5.1$, $\sigma = 0.24$, $\lambda = 0.1960$

%	
%	NEED TO SHOW VISUALISATIONS OF IRIS HERE AND DISCUSS TWO INITIALISATIONS: ONE OVERFITTING AND ONE UNDERFITTING, AND SHOW THAT OUR TRAINING ALGORITHM FINDS A BALANCE FROM BOTH INITIAL SETTINGS.
%	
%	ALSO SHOW REVERSE EMBEDDING RESULTS
	
	\paragraph{Neural Network Kernels} Our learning algorithm is general and can learn the hyperparameters of any conditional embedding with discrete target space for any family of positive definite kernels $k_{\theta}$ over $\theta \in \Theta$ and $\lambda \in \Lambda$. One particularly useful class of kernels are neural network kernels, in which the kernel is formed from explicit features that come from (usually but not necessarily the last layer of) a neural network. These kernels benefit in both scalability and expressibility. For large datasets, the $n \times n$ Cholesky decomposition for full gradient update in \cref{alg:kernel_embedding_classifier_training} can be replaced by a $p \times p$ matrix that is independent of the size of $n$ by the Woodbury matrix inversion identity \citep{higham2002accuracy}. This allows scalability for $n >> p$ without using stochastic gradient updates so that the bound \eqref{eq:expected_risk_bound_hyperparameter_learning_copy} can be as tight as possible for a given training set of size $n$. Neural networks are by construction compositional, and thus latent representations of deeper networks undergo more transformations to generates a smaller (not infinitely many) collection of more expressive nonlinear features. That is, typical nonlinear kernels define implicit high dimensional features and thus has a shallow but wide model architecture. In contrast, neural network kernels define explicit lower dimensional features through multiple layers of abstraction, and thus has a deeper but narrower model architecture. We direct the reader to the supplementary material for a detailed discussion of the various kernel embedding classifier architectures. The feature transformations are typically defined by weights and biases of each hidden layer, which define the parameters $\theta \in \Theta$ of a neural network kernel and are thus trainable under our learning algorithm. 
...
\label{sec:experiments}
	...
	\begin{table}[t]
		...
		\begin{tabular}{lccccc}			
			...
			\texttt{robot} \hspace{\fill} (5456, 24, 4) & $97.8$ & $95.5 \pm 0.9$ & $82.3 \pm 7.1$ & $94.5 \pm 0.8$ & 97.59\textsuperscript{c} \\
			\texttt{segment} \hspace{\fill} (2310, 19, 7) & $98.4 \pm 0.8$ & $96.1 \pm 1.5$ & $94.6 \pm 1.6$ & $96.7 \pm 1.1$ & 96.83\textsuperscript{d} \\
			\texttt{waveform} \hspace{\fill} (5000, 21, 3) & $87.2$ & $83.3 \pm 1.6$ & $86.8 \pm 1.6$ & $86.7 \pm 1.6$ & 90.98\textsuperscript{d} \\
