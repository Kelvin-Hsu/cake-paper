...
\label{sec:kernel_embedding_classifier}
	...
	\begin{proof}
		...
		\note{Note: The proof can just stop right here really. Every time we have some estimator error being upper bounded by some constant (in this case, $1$) times the RKHS norm of the difference between the empirical and true conditional embeddings, we can simply say that if the right hand side goes to zero at a stochastic rate of $O_{p}((n \lambda)^{-\frac{1}{2}} + \lambda^{\frac{1}{2}})$, then so does the left hand side. The constant needs to be independent of $n \in \mathbb{N}_{+}$ for this to be true. If the constant is independent of $x \in \mathcal{X}$, then the convergence is uniform. If not, then the convergence is non-uniform.}
...
\label{app:theorems_derivations}
	...
	\label{thm:probability_vector_convergence} 
		...
		\begin{proof}
			For convergence in $\ell_{1}$, we simply extend \cref{thm:probability_convergence}, which proved that each entry of $\hat{\bvec{p}}(x)$ converges pointwise uniformly at a rate of $O_{p}((n \lambda)^{-\frac{1}{2}} + \lambda^{\frac{1}{2}})$ to the corresponding entry of $\bvec{p}(x)$. The $\ell_{1}$-norm of the difference is simply
			
			\begin{equation}
				{\lVert \hat{\bvec{p}}(x) - \bvec{p}(x) \rVert}_{\ell_{1}} = \sum_{c = 1}^{m} | \hat{p}_{c}(x) - p_{c}(x) |.
			\end{equation}
			
			Since each term in the sum converges stochastically at a rate of $O_{p}((n \lambda)^{-\frac{1}{2}} + \lambda^{\frac{1}{2}})$, then so does the sum, which proves the pointwise uniform convergence in $\ell_{1}$.
			...
			where the last equality comes from \eqref{eq:rkhs_norm_is_l2_norm} and the fact that the empirical and true conditional embeddings are bounded functions in the RKHS. Since this is true for all $x \in \mathcal{X}$ and the rate does not depend on $x \in \mathcal{X}$, the convergence is pointwise uniform.
		\end{proof}
