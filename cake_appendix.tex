...
\begin{document}
	...
	\begin{theorem}[Uniform Convergence of Empirical Decision Probability Function]
		...
		\begin{proof}
			...
			From \eqref[name = , xr=cake]{eq:indicator_function}, since $\mathbb{1}_{c} = \delta(c, \cdot)$ and using the fact that $\delta$ is a reproducing kernel, we have that for all $c \in \mathcal{Y} = \mathbb{N}_{m}$.
			\begin{equation}
			\begin{aligned}
	...
	\Cref{thm:empirical_probability_is_embedding} shows that the decision function $\bvec{f}(x)$ \eqref[name = , xr=cake]{eq:empirical_decision_probability_vector} of a kernel embedding classifier is no more than the empirical conditional embedding estimated from the data.
	...
	\label{thm:probability_vector_convergence} 
		...
		\begin{proof}
			...
			where the last equality comes from \eqref[name = ]{eq:rkhs_norm_is_l2_norm} and the fact that the empirical and true conditional embeddings are bounded functions in the RKHS. Again, by \cref{thm:pointwise_uniform_convergence} with $\gamma(x) = 1$ independent of $x \in \mathcal{X}$, we have uniform convergence in $\ell_{2}$.
		\end{proof}
	\end{lemma}
	...
	\label{app:information_entropy}
		...
		The first approach is straight forward, which involves simply computing the information entropy with the clip normalised probabilities \eqref[name = , xr=cake]{eq:empirical_decision_probability_clip_normalised}, at the query point $x \in \mathcal{X}$,
		...
		We call \eqref[name = ]{eq:entropy_clip_normalise} the \textit{clip-normalised information entropy}. Since $\tilde{p}_{c}(x)$ converges pointwise to $p_{c}(x)$ with increasing data, $\tilde{h}(x)$ also converges pointwise to $h(x)$.
		...
		\begin{theorem}[Convergence of Empirical Information Entropy Function]
			...
			\begin{proof}
				...
				where we define $\alpha_{x} := \max_{c \in \mathbb{N}_{m}} \frac{1}{| \mu_{Y | X = x}(c) |}$, which is well defined as the conditional embedding is bounded. Since the RKHS norm of bounded functions in $\mathcal{H}_{\delta}$ is simply the $\ell_{2}$-norm of their vector representations \eqref[name = ]{eq:rkhs_norm_is_l2_norm}, we have
...
\label{app:learning_theoretic_bounds}
	...
	\label{app:expected_risk_bounds}
		...
		We will refer to \eqref[name = ]{eq:normalised_cross_entropy_loss} as the normalised cross entropy loss. We then further define the centred normalised cross entropy loss,
		...
		With the normalised cross entropy loss \eqref[name = ]{eq:normalised_cross_entropy_loss} as our loss function, we now employ Theorem 8 of \cite{bartlett2002rademacher} for this loss and provide a bound for the expected normalised cross entropy loss for an unseen test example.
		...
		\begin{lemma}[General Expected Risk Bound for the Kernel Embedding Classifier]
			...
			for any $\epsilon \in (0, 1)$. Equivalently, the bound \eqref[name = ]{eq:expected_loss_bound_for_kec} holds for $f = \bvec{f}_{\theta, \lambda}(x)$ for every $\theta \in \Theta, \lambda \in \Lambda$.
		...
		\begin{theorem}[Specific Expected Risk Bound for the Kernel Embedding Classifier]
			...
			for any $\epsilon \in (0, e^{-1})$. Equivalently, the bound \eqref[name = ]{eq:specific_expected_loss_bound_for_kec} holds for $f = \bvec{f}_{\theta, \lambda}(x)$ for every $\theta \in \Theta, \lambda \in \Lambda$.
	...
	\label{app:expected_risk_bounds_for_hyperparameter_learning}
		...
		\begin{lemma}[General Expected Risk Bound for KEC Hyperparameter Learning]
			...
			\begin{proof}
				We first apply \cref{thm:specific_expected_loss_bound_for_kec} with the choice of $\rho = \| W_{\tilde{\theta}, \tilde{\lambda}} \|_{\mathrm{tr}}$ and $\alpha^{2} = \sup_{x \in \mathcal{X}} k_{\tilde{\theta}}(x, x)$. The inequality \eqref[name = ]{eq:specific_expected_loss_bound_for_kec} then only holds for a subset of kernel parameters and regularisations $(\theta, \lambda) \in \Xi(\tilde{\theta}, \tilde{\lambda})$ as defined by \eqref[name = ]{eq:hyperparameter_subset_definition}.
			\end{proof}
		\end{lemma}
	...
	\label{app:explicit_kernel_embedding_classifier}
		...
		\begin{algorithm}[tb]
			...
			\begin{algorithmic}[1]
				...
				\STATE $q(\theta, \lambda) \leftarrow \frac{1}{n_{b}} \sum_{i \in \mathcal{I}_{b}} \mathcal{L}_{\epsilon}((Y)_{i}, (P_{\theta, \lambda})_{i}) + 4 e \; \alpha(\theta) \sqrt{\sum_{c = 1}^{m} \sum_{j = 1}^{p} (W_{\theta, \lambda})_{j, c}^{2}}$
